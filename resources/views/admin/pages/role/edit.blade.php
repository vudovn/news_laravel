...
@section('content')
    ...
    <div class="d-flex flex-column gap-4">
        ...
        <form action="{{ route('admin.role.update', $role->id) }}" method="POST" class="row g-3 needs-validation" novalidate="">
            @csrf
            <div class="col-lg-12 col-12">
                <label for="name">Tên vai trò</label>
                <input type="text" name="name" id="name" value="{{ $role->name ?? old('name') }}"
            ...
            <div class="col-lg-12 col-12">
                ...
                <input onchange="checkAllPermission()" type="checkbox" class="form-check-input check_all_permission" id="checkAll">
                    ...
                    @foreach ($formattedPermissionsGroup as $key => $item)
                        <div class="col-3 mb-5">
                            ...
                            @foreach ($item['permissions'] as $permission)
                                <input {{ in_array($permission->name, $rolePermissions) ? 'checked' : '' }} type="checkbox" name="permissions[]" class="form-check-input checked_permissions"
